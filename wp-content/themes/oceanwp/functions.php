...
function restrict_dashboard_login_page() {
    if ( is_page( 'dashboard-aihomework-helper' ) && ! is_user_logged_in() ) {
        wp_redirect( wp_login_url() );
        exit;
    }
}
add_action( 'template_redirect', restrict_dashboard_login_page );

function redirect_to_dashboard_login( $redirect_to, $request, $user ) {
    // Check if the user is logged in
    if ( isset( $user->roles ) && is_array( $user->roles ) ) {
        // Redirect to "dashboard-login" if logged in
        return home_url( '/dashboard-aihomework-helper/' );
    } else {
        return $redirect_to;
    }
}
add_filter( 'login_redirect', 'redirect_to_dashboard_login', 10, 3 );
